	...
	implements  Serializable
{
	...
	{
		vFluents.put( "INREQUEST_B", generatedbyassl.as.aes.user_b.aeself_management.self_configuring.INREQUEST_B.getInstance() );
	}
	...
	{
		//************************ MAPPING ***********************
		if ( generatedbyassl.as.aes.user_b.aeself_management.self_configuring.INREQUEST_B.getInstance().isFluentInitiated() ) 
		{
			...
			Runnable actionsRunner = new Runnable()
			{
				...
				public void run (  )
				{
					try
					{
						generatedbyassl.as.aes.user_b.actions.REQUEST.getInstance().perform(  );
					}
					catch ( Exception ex ) 
					{
			...
			if ( !isMappingRunning( "INREQUEST_B" ) ) 
			{
				Thread runActions = new Thread(thrdgrMappings, actionsRunner, "INREQUEST_B");
				runActions.start();
			}
		}
	}
